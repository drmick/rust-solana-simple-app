...
impl CMCService {
    pub async fn get_btc_price(&self) -> f32 {
        ...
        let resp = client
            ...
            .header("X-CMC_PRO_API_KEY".to_string(), &self.api_key)
            .send()
            .await
            .unwrap();
        let resp = resp.json::<CMCResponse>().await.unwrap();

        let price: f32 = resp
            .data
            .iter()
            .filter(|it| it.symbol == "BTC".to_string())
            .map(|it| it.quote.get("USD").unwrap().price)
            .next()
            .unwrap();
